...
/*
    Instant Meshes application command line interface
    Default setting uses triagle mesh output with mesh dominant
*/
...
int main(int argc, char **argv) {
    ...
    // orignally dominant = false
    bool extrinsic = true, dominant = true, align_to_boundaries = false;
    bool fullscreen = false, help = false, deterministic = false, compat = false;
    // rosy 6, posy 3 setting mean triagle mesh output
    // orignal 4, 4 results in quad mesh
    ...
    if (args.size() == 0)
        cout << "Running in CLI mode, start with -h for instructions on batch mode." << endl;
